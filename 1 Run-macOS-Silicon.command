#!/usr/bin/env bash
# AutoEditor — run the app on macOS (Apple Silicon, arm64).
#
...
command -v node >/dev/null 2>&1 || die "Node.js is not installed. Get it from https://nodejs.org (LTS, macOS arm64 / Apple Silicon) and run this again."

ARCH=$(uname -m)
case "$ARCH" in
  arm64)  SAY_ARCH="Apple Silicon" ;;
  x86_64) SAY_ARCH="Intel"
          info "Note: this script targets Apple Silicon (arm64). You're on Intel —"
          info "the ffmpeg binary will be reinstalled for x86_64, which still works."
          ;;
