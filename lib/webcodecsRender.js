...
import { captionCueAt, drawCaption, captionFontPx, drawWatermark } from "./captions";
...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  const { clips, fps = 30, transitions = [], transitionDuration = 0.4, motions = [], motionAmount = 0.08, fx = [], fxAmount = 0.5,
    ...
    videosByName = {}, trims = [], speeds = [], volumes = [], bitrate = null, audioLayers = [],
    overlayUrl = null, overlayDuration = 0, overlayOpacity = 0.3, overlayBlendMode = "overlay", overlayLoop = true, overlayEnabled = false,
    watermarkUrl = null, watermarkSize = 0.15, watermarkX = 0.8, watermarkY = 0.88, watermarkOpacity = 0.9, watermarkEnabled = false } = spec;
  ...
  let audioEnc = null, audioErr = null, audioOff = 0, aChunks = 0, audioDead = !wantAudio, clipMixes = [], layerMixes = [];
  if (wantAudio) {
    try { clipMixes = await prepareClipMixes(); } catch (_) { clipMixes = []; }
    try { layerMixes = await prepareLayerMixes(); } catch (_) { layerMixes = []; }
    if (!voiceSource && !clipMixes.length && !layerMixes.length) {
      audioDead = true;
