...
// ---- meta flags -------------------------------------------------------------
// Small app-wide key/value flags (e.g. "first-run tour already shown") that
// live in the same IndexedDB as projects, so they survive browser restarts
// even when localStorage is unavailable or gets cleared.

export async function getMeta(key) {
  try { return await tx(STORE_META, "readonly", (s) => reqP(s.get(key))); }
  catch (_) { return null; }
}

export async function setMeta(key, value) {
  try { await tx(STORE_META, "readwrite", (s) => reqP(s.put(value, key))); } catch (_) {}
}
